...
export const initializeExampleData = () => {
  // Clear all existing users except fioalvira1809@gmail.com
  const existingUsers = JSON.parse(localStorage.getItem('curriculum-users') || '[]');
  const filteredUsers = existingUsers.filter((u: any) => u.email === 'fioalvira1809@gmail.com');
  
  // If fioalvira1809@gmail.com doesn't exist, create it
  let fioUser = filteredUsers.find((u: any) => u.email === 'fioalvira1809@gmail.com');
  if (!fioUser) {
    fioUser = {
      id: 'fioalvira1809-user-id',
      email: 'fioalvira1809@gmail.com',
      password: 'example123', // In production, this should be hashed
    };
    filteredUsers.push(fioUser);
  }
  
  // Save cleaned users list
  localStorage.setItem('curriculum-users', JSON.stringify(filteredUsers));

  // Clean templates to only keep templates for existing users
  const existingTemplates = JSON.parse(localStorage.getItem('curriculum-templates') || '[]');
  const validUserIds = filteredUsers.map((u: any) => u.id);
  const filteredTemplates = existingTemplates.filter((t: CurriculumTemplate) => 
    validUserIds.includes(t.userId)
  );

  // Check if fioalvira1809@gmail.com has the default template
  const fioTemplate = filteredTemplates.find((t: CurriculumTemplate) => 
    t.userId === fioUser.id && t.careerName === 'Ingeniería en Sistemas'
  );

  if (!fioTemplate) {
    // Create default template with the original ORT curriculum for fioalvira1809@gmail.com
    ...
    const defaultTemplate: CurriculumTemplate = {
      id: 'default-fioalvira1809-template',
      userId: fioUser.id,
    ...
    filteredTemplates.push(defaultTemplate);
  }

  localStorage.setItem('curriculum-templates', JSON.stringify(filteredTemplates));
};
